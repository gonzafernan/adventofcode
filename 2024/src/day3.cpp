...
int day3_part1(const string &memory_corrupted) {
    ...
    auto matches_begin = sregex_iterator(memory_corrupted.begin(), memory_corrupted.end(), pattern);
...
int day3_part2(const string &memory_corrupted) {
    ...
    for (auto match_it = matches_begin; match_it != matches_end; match_it++) {
        ...
        if (match_op[OP_DO_IDX] != "") {
            ...
            ans += stoi(match_op[OP_MULT_VALUE1_IDX]) * stoi(match_op[OP_MULT_VALUE2_IDX]);
        }
    }
    return ans;
}

int day3(string input_file) {
